...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def shortest_forwarding(self, msg, eth_type, ip_src, ip_dst):
	   ...
	   path1, path2 = self.get_shortest_path(src_sw,dst_sw)
           if src_sw != dst_sw:
		sw_pos_in_path=path1.index(datapath.id)
                if sw_pos_in_path+1 != len(path1):
                	nextsw=path1[sw_pos_in_path+1]
                        edge_attrib=nx.get_edge_attributes(self.net,'weight')
                        path1outport=edge_attrib[(datapath.id,nextsw)]['port']
	   	sw_pos_in_path=path2.index(datapath.id)
                if sw_pos_in_path+1 != len(path2):
                	nextsw=path2[sw_pos_in_path+1]
                	edge_attrib=nx.get_edge_attributes(self.net,'weight')
                        path2outport=edge_attrib[(datapath.id,nextsw)]['port']
		if not dst_sw:
	 		self.send_group_mod(datapath, 1, path1outport, path1outport, 
				path2outport, path2outport)
			self.build_flow(datapath, 1, flow_info, in_port, 1)
			self.send_packet_out(datapath, msg.buffer_id, in_port, 
				path1outport, msg.data)
		else:
			self.build_flow(datapath, 1, flow_info, in_port, path1outport)
			self.send_packet_out(datapath, msg.buffer_id, in_port, 
				path1outport, msg.data)
		if not src_sw:
			self.send_group_mod(datapath, 2, path2outport, path2outport, 
				path1outport, path1outport)
			self.build_flow(datapath, 1, back_info, path2outport, 2)
			self.send_packet_out(datapath, msg.buffer_id, in_port, 
				path2outport, msg.data)
		else:
			self.build_flow(datapath, 1, back_info, path2outport, in_port)
                	self.send_packet_out(datapath, msg.buffer_id, in_port, 
				path2outport, msg.data)
	else:
        	out_port = self.get_port(flow_info[2], self.access_table)
            	...
            	self.send_packet_out(datapath, msg.buffer_id, in_port, out_port, 
			msg.data)      
        return
    ...
    def get_shortest_path(self,src,dst):
        total_edges = self.net.edges
        path1 = nx.shortest_path(self.net,src,dst)
        edges_list_path1 = self.pairwise(path1)
        edges_remain = set(total_edges) - set(edges_list_path1)
