...
def runner():
    ...
    for i in range(25):
        ...
        while node1 == node2:
            node2 = random.randint(1,10)
        test_pair.append((node1,node2))
    result={'src':[],'dst':[],'hop':[],'delay':[],'throughput':[],'datasize':[],'avg_time':[]}
    time.sleep(0.2)
    for pair in test_pair:
        ...
        try:
           stats = ping(net,[src,dst],64,2)# send some packets before calculation
           print(stats)
           avg_time = stats[3]
        ...
        try:
            ...
            print('Trying again')
            continue
        result['src'].append(host1)
        result['dst'].append(host2)
	...
	result['datasize'].append(datasize_tx)
        result['avg_time'].append(avg_time)
        fields = [result['src'][-1], result['dst'][-1], result['hop'][-1], result['delay'][-1], result['throughput'][-1], result['datasize'][-1], result['avg_time'][-1]]
        with open('gooddataset.csv', 'a') as file:
