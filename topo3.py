...
def runner():
    ...
    net.addController(c)             
    net.start() 
    test_pair=[(1,7),(3,8),(1,9),(8,6),(9,1)]
    result={'hop':[],'delay':[],'throughput':[]}
    for pair in test_pair:
        host1,host2=pair
        src=net.getNodeByName('h'+str(host1+1))
        dst=net.getNodeByName('h'+str(host2+1))
	#Could not get delete link function to work
	#src_sw = net.getNodeByName('s'+str(host1+1))
	#dst_sw = net.getNodeByName('s'+str(host2+1))
	net.delLinkBetween(src_sw,dst_sw)
