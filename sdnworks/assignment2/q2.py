...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def add_flow(self, datapath, priority, match, actions,idle_timeout=0, hard_timeout=0):
        ...
        inst = [parser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS,
                                             actions)]
        inst2=[parser.OFPInstructionGotoTable(1)]
        if priority!=0:
               mod = parser.OFPFlowMod(datapath=datapath,table_id=1,
                                    priority=priority, match=match,
                                    instructions=inst)
               mod1 = parser.OFPFlowMod(datapath=datapath,table_id=0,
    ...
    def build_flow(self, datapath,priority, flow_info, src_port, dst_port):
        parser = datapath.ofproto_parser
        actions = []
        if flow_info[1] in ['10.0.0.6','10.0.0.7'] or flow_info[2] in ['10.0.0.6','10.0.0.7']:
           actions = []
        else:
    ...
    def shortest_forwarding(self, msg, eth_type, ip_src, ip_dst):
        ...
        if result:
           ...
           print result, in_port, 'dpid:',datapath.id	
           if src_sw != dst_sw:
              if dst_sw:               
